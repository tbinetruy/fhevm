    ...
    Ownable2StepUpgradeable
{
    ...
    /// @dev Override totalSupply to handle ETH (address(0))
    function totalSupply() public view virtual override returns (uint256) {
        if (underlying() == address(0)) {
            // For ETH, use contract's ETH balance
            return address(this).balance / rate();
        } else {
            // For ERC20, use parent implementation
    ...
    function wrapETH(address to, uint256 amount) public payable virtual {
    ...
    function finalizeUnwrap(
        ...
        _transferUnderlying(to, burntAmountCleartext * rate());
    ...
    function _transferUnderlying(address to, uint256 amount) internal returns (bool success) {
        if (underlying() == address(0)) {
            ...
            success = SafeERC20.trySafeTransfer(IERC20(underlying()), to, amount);
        }
    }
}
